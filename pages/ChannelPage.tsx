...
export const ChannelPage: React.FC = () => {
  ...
  const { programs, loading: epgLoading, error: epgError } = useEpg(channel?.id);
    ...
    return (
      <div className="flex-grow flex items-center justify-center text-center p-4">
        <div>
            ...
            <button onClick={() => navigate('/')} className="px-6 py-2 bg-brand-primary text-white rounded-md font-semibold hover:bg-sky-400 transition-colors">
                {t('goHome')}
            </button>
        </div>
      </div>
      ...
      <div className="mt-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <div className="lg:col-span-2">
                  <EpgDisplay programs={programs} loading={epgLoading} error={epgError} />
              </div>
              <div className="lg:col-span-1">
                  <ProgramGuide programs={programs} loading={epgLoading} error={epgError} />
              </div>
          </div>
      </div>
    </div>
